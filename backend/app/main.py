...
from app.seed import seed_categories, seed_cities


@asynccontextmanager
async def lifespan(_: FastAPI):
    ctxt = RepositoryContext()  # creates the metadata, so the data can be seeded
    await ctxt.init_db()
    await seed_categories()  # seed categories on startup
    await seed_cities()  # seed cities on startup
    yield
